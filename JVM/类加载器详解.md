...
如果需要打破双亲委派模型，则ClassLoader的loadClass方法，例如Tomcat则打破了双亲委派模型，通过自定义类加载器WebAppClassLoader打破双亲委派，即重写了JVM的类加载器ClassLoader的findClass方法和loadClass方法，以优先加载Web应用目录下的类。每个Web模块单独使用一个WebAppClassLoader，防止了多个Web模块因为引用库的版本不同从而冲突。
